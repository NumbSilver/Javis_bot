...
- **status=extracted（值得启动 M5）**：满足以下**任一**条即可：
  1. principal 直接要求 Jarvis 做的事——包括在群里点名 jarvis 让它说话、回复、查询或执行。这个判断不依赖飞书真实 mention，也不依赖 Jarvis Bot 当时是否已在群里；名称大小写差异及 `Javis` 这类明显拼写误差仍视为 Jarvis。这类指令本身就是准入依据，**不再评估它有没有业务价值、是否琐碎、是否像测试或玩笑**；
